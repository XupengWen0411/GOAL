...
public class RandomMapCreator {
    ...
    public static ZoneModel[][] createRandomGrid(int rows, int cols, int roomCount) {
        ...
        initGrid(grid);
        createAreas(grid);
        int amountPossibleAreas = maxAreasPossible(rows, cols);
        if (amountPossibleAreas < roomCount) {
        ...
        List<Node> allAreas = getAreas(grid);
        List<Node> allCorridors = getCorridors(grid);
        randomizeCorridors(allCorridors);
        randomizeAreas(allAreas, amountPossibleAreas - roomCount);
        randomizeDoorDirs(allAreas);
     ...
     */
    public static int maxAreasPossible(int r, int c) {
        return (c - 2)*(r/2);
    }
     ...
     * @param changedAreas The amount of rooms that need to be reclassified.
     */
    private static void randomizeAreas(List<Node> rooms, int changedAreas) {
    ...
    private static void selectRandomDropZone(ZoneModel[][] grid) {
        List<ZoneModel> list = getAreas(grid);
        Random r = new Random(System.currentTimeMillis());
        int index = r.nextInt(list.size());
     ...
     */
    private static List<ZoneModel> getAreas(ZoneModel[][] grid) {
        List<ZoneModel> l = new LinkedList<ZoneModel>();
        for (int i = 0; i < grid.length; i++) {
            for (int j = 0; j < grid[0].length; j++) {
     ...
     * @return A list containing every tile in the grid classified as a room.
     */
    private static List<Node> getAreas(Node[][] grid) {
        List<Node> l = new LinkedList<Node>();
        for (int i = 0; i < grid.length; i++) {
            for (int j = 0; j < grid[0].length; j++) {
                if (grid[i][j].getType() == Zone.Type.ROOM) {
     ...
     */
    private static void createAreas(Node[][] grid) {
