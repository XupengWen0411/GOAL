...
public class MapConverter {
    ...
    public static NewMap createMap(EnvironmentMap model) throws MapFormatException {
        ...
        map.setArea(new Point(model.getColumns() * MapConverter.ROOMWIDTH, model.getRows() * MapConverter.ROOMHEIGHT,0));
            ...
            for (int col = 0; col < model.getColumns(); col++) {
                ...
                if (output[row][col].getType() == Type.ROOM) {
                    if (room.hasDoor(ZoneModel.NORTH)) {
                        output[row][col]
                                .addDoor(new Door(new Point(x, y - ROOMHEIGHT / 2,0), Door.Orientation.HORIZONTAL));
                    }
                    if (room.hasDoor(ZoneModel.EAST)) {
                        output[row][col].addDoor(new Door(new Point(x + ROOMWIDTH / 2, y,0), Door.Orientation.VERTICAL));
                    }
                    ...
                    if (room.hasDoor(ZoneModel.WEST)) {
                        output[row][col].addDoor(new Door(new Point(x - ROOMWIDTH / 2,0, y), Door.Orientation.VERTICAL));
                    }
                }
     ...
     *            indicates where the current zone is
     */
    private static void connectArea(Zone[][] zones, int row, int col) {
    ...
    private static void addEntities(NewMap map, double centerx, double centery, int numberOfEntities) {
        for (int n = 1; n <= numberOfEntities; n++) {
            ...
            Point p;
            if (n4 == 1) {
                p = new Point(centerx - 2.5, centery - 2.5,0.0);
            } else if (n4 == 2) {
                p = new Point(centerx + 2.5, centery - 2.5,0.0);
            } else if (n4 == 3) {
                p = new Point(centerx - 2.5, centery + 2.5,0.0);
