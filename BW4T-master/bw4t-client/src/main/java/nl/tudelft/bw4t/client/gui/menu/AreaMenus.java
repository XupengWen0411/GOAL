...
import nl.tudelft.bw4t.client.gui.listeners.GoToAreaActionListener;
...
public class AreaMenus {
	...
	public static void buildPopUpMenuForBeingAtBlock(ViewBlock box, Zone room,
			BW4TClientGUI gui) {
		AreaMenus.controller = gui.getController();
	...
	public static void buildPopUpMenuForBlock(ViewBlock box, Zone room,
		...
		AreaMenus.controller = controller;
	 ...
	 */
	public static void buildPopUpMenuArea(Zone room, BW4TClientGUI gui) {
		AreaMenus.controller = gui.getController();
		...
		menuItem.addActionListener(new GoToAreaActionListener(label, controller));
		gui.getjPopupMenu().add(menuItem);
		...
		// option for each amount of blocks (e.g.: Area X contains -> 3 -> Pink)
		for (int i = 0; i < 6; i++) {
		...
		if (holding != null) {
			...
			for (Zone roomInfo : cmc.getAreas()) {
