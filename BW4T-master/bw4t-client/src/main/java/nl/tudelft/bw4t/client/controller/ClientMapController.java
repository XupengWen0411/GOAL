...
public class ClientMapController extends AbstractMapController {
	...
	/** The occupied rooms. */
	private final Set<Zone> occupiedAreas = new HashSet<Zone>();
	 ...
	 * @return
	 */
	public synchronized Set<Zone> getOccupiedAreas() {
		return new HashSet<Zone>(occupiedAreas);
	}
	...
	public synchronized boolean isOccupied(Zone room) {
		...
		return occupiedAreas.contains(room);
	}
	 ...
	 *            the name
	 */
	public synchronized void removeOccupiedArea(Zone zone) {
		if (!zone.isInitialized()) {
			throw new IllegalStateException("zone is not initialized:" + zone);
		}
		occupiedAreas.remove(zone);
	}
