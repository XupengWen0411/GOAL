...
public class MapRenderer extends JPanel implements MapRendererInterface {
	...
	public void paintComponent(Graphics g) {
		...
		updateMinimumSize();
//		Image img = Toolkit.getDefaultToolkit().getImage("D:\\JavaWorkspace\\BW4T-master\\BW4T-master\\bw4t-core\\moon.jpg");
		Image img = new ImageIcon("moon.jpg").getImage();
...
//		drawLabels(g2d);
	 ...
	 *
	 * @param g2d
	 *            t Graphics2D where will be drawn
	 */
	public void drawAreas(Graphics2D g2d) {
		MapRenderSettings set = getController().getRenderSettings();

		for (Zone room : getController().getAreas()) {
...
//				drawDoor(g2d, door, occupied);
	 ...
	 * @param closed
	 *            based on if the door is closed or not, the color will be
	 *            determined
	 */
...
//					.getX(), loc.getY(), e.getRobotSize(), e.getRobotSize())));
			...
			Rectangle2D r2d = set.transformCenterRectangle(new Rectangle2D.Double(loc
					.getX(), loc.getY(), e.getRobotSize(), e.getRobotSize()));
			int nPoints=3;
			int[] xPoint1s={(int)r2d.getX()+3,(int)r2d.getX(),(int)r2d.getX()-3};
			int[] yPoint1s={(int)r2d.getY(),(int)r2d.getY()-6,(int)r2d.getY()};
			g2d.drawPolygon(xPoint1s, yPoint1s, nPoints);
			int[] xPoint2s={(int)r2d.getX()+6,(int)r2d.getX(),(int)r2d.getX()-6};
			int[] yPoint2s={(int)r2d.getY()+4,(int)r2d.getY(),(int)r2d.getY()+4};
			g2d.drawPolygon(xPoint2s, yPoint2s, nPoints);
